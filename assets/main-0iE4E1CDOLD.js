...
window.dropdownsInit = function() {
  ...
  dropdonws.forEach((holder) => {
    ...
    holder.addEventListener("click", function(event2) {
    ...
    links.forEach(function(link, index) {
      ...
      link.addEventListener("click", function(event2) {
        ...
        if (link.classList.contains("active")) {
          dropdowns.forEach(function(dropdown, index2) {
            dropdown.classList.remove("active");
...
window.filterMenu = function() {
  ...
  filterMenuBtn.addEventListener("click", function() {
    if (filterMenuBody.classList.contains("hidden")) {
      filterMenuBody.classList.remove("hidden");
      filterMenuBody.classList.add("flex");
      document.body.classList.add("overflow-hidden");
    } else {
      filterMenuBody.classList.remove("flex");
      filterMenuBody.classList.add("hidden");
      document.body.classList.remove("overflow-hidden");
    }
  });
  filterMenuClose.addEventListener("click", function() {
    if (filterMenuBody.classList.contains("hidden")) {
      filterMenuBody.classList.remove("hidden");
      filterMenuBody.classList.add("flex");
      document.body.classList.add("overflow-hidden");
...
window.sortItems = function() {
  ...
  const sortButtons = sortGroup.querySelectorAll("[data-sort-item]");
...
window.filterItems = function() {
  ...
  const filterButtons = filterGroup.querySelectorAll('[data-filter-item]:not([data-filter-item=""])');
  const allFilterButton = filterGroup.querySelector('[data-filter-item=""]');
...
window.likeItems = function() {
  const likeButtons = document.querySelectorAll("[data-like-item]");
...
window.inputFileName = function() {
  const inputFile = document.getElementById("inputFile");
  const inputFileTitle = document.querySelector(".input-file__title");
  const inputFileSubtitle = document.querySelector(".input-file__subtitle");
  const clearButton = document.querySelector(".input-file__clear");
  inputFile.addEventListener("change", function() {
    const filename = inputFile.files.length ? inputFile.files[0].name : "";
    if (inputFile.files.length > 0) {
      inputFileSubtitle.textContent = filename;
      inputFileTitle.classList.add("hidden");
      clearButton.classList.remove("hidden");
    } else {
      inputFileTitle.textContent = "Приложить файл";
      inputFileSubtitle.textContent = "PDF, JPG, PNG, WORD";
      inputFileTitle.classList.remove("hidden");
...
document.addEventListener("DOMContentLoaded", function(event2) {
  ...
  if (document.querySelectorAll(".slider-filter").length) {
    window.swiperInit();
  }
...
window.preloader = {
  add: function(target) {
    ...
    if (target) {
      ...
      preloader.classList.add("preloader--sm");
      preloader.classList.remove("preloader");
      inner.innerHTML = `<div class="w-4 h-4"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      preloader.prepend(inner);
      target.prepend(preloader);
    } else {
      document.body.classList.add("loading");
      inner.classList.add("text-orange-roughy");
      inner.innerHTML = `<div class="w-12 h-12"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M1 12C1 18.0751 5.92487 23 12 23C18.0751 23 23 18.0751 23 12C23 5.92487 18.0751 1 12 1" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
					<animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="0.6s" repeatCount="indefinite"/>
				</path>
			  </svg></div>`;
  ...
  remove: function() {
    var prelodares = document.querySelectorAll(".preloader, .preloader--sm");
